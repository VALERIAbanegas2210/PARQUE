...
@section('content')
    ...
    <div class="container mt-5">
        ...
        <div id="container-montoTotal" class="d-flex justify-content-start mb-4">
            @isset($montoTotal)
                <h6>Total A Pagar: {{$montoTotal}} Bs</h6>
            @endisset
        </div>
        <div class="d-flex justify-content-between">
            <!--para volver atras-->
            <button id="goBackButton" class="btn btn-secondary">Volver Atrás</button>
            <form action="{{ route('admin.formularios.guardarReservas') }}" method="POST">
                @csrf
                <input type="hidden" name="formData" value="{{ json_encode($formData) }}">
                <input type="hidden" name="montoTotal" value="{{ $montoTotal }}"><!--Envio del monto total oculto-->
                <!--si hubo cambios o los cambios estan en nulos entonces que este desabilitado-->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            //console.log(`es actualizacion ${@json($esActualizacion)}`);
            //console.log(@json($huboCambios))    
            ...
            if (!savedData) {
                ...
                //modificarEstadoBotonConfirmar(); // Llama a la función después de inicializar los datos
            }
        });
        ...
        function cargarDatos(formData) {
            ...
            formData.forEach((data, index) => {
                ...
                let tipoPagoHTML = '';
                if (data.tipoPago) {
                    tipoPagoHTML += `<li><strong>Tipo Pago:</strong> ${data.tipoPago}</li>`;
                    if (data.tipoPago === "Mixto") {
                        tipoPagoHTML += `
                            <li><strong>Monto QR:</strong> ${data.montoQR || '0'} Bs</li>
                            <li><strong>Monto Efectivo:</strong> ${data.montoEfectivo || '0'} Bs</li>
                        `;
                    } else {
                        tipoPagoHTML += `<li><strong>Monto Pagado:</strong> ${data.montoRelativo || '0'} Bs</li>`;
                    }
                }
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <ul>
                                    ...
                                    ${tipoPagoHTML} <!-- Aquí se incluye la lógica del tipo de pago -->
